...
<center>
![Figure 1: Pipeline flow chart](https://github.com/Palmer-Lab-UCSD/HS-Rats-Genotyping-Pipeline/master/assets/pipeline_overview.png){#id .class width=80% height=80%}  
...
## BEAGLE SNPs Hardy–Weinberg P value vs. minor allele frequency  
X axis: SNPs minor allele frequency on genotypes called by STITCH and imputed by BEAGLE (extract using plink2)  
Y axis: SNPs -log10(Hardy–Weinberg P value) on genotypes called by STITCH and imputed by BEAGLE (extract using plink2)  
The effects on the called genotypes' reliability due to the heterozygote and rare-homozygote clusters of the genotype calling algorithm.  
<center>
![Figure 15: BEAGLE SNPs Hardy–Weinberg P value vs minor allele frequency](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_SNPs_hwe_vs_maf.png")`){#id .class width=60% height=60%}

</center>

## BEAGLE polymorphic SNPs Hardy–Weinberg P value vs. minor allele frequency  
X axis: SNPs minor allele frequency on genotypes called by STITCH and imputed by BEAGLE (extract using plink2)  
Y axis: SNPs -log10(Hardy–Weinberg P value) on genotypes called by STITCH and imputed by BEAGLE (extract using plink2)  
The effects on the called genotypes' reliability due to the heterozygote and rare-homozygote clusters of the genotype calling algorithm.   
BEAGLE polymorphic SNPs: MAF > 0.005
<center>
![Figure 16: BEAGLE polymorphic SNPs Hardy–Weinberg P value vs minor allele frequency ](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_poly_SNPs_hwe_vs_maf.png")`){#id .class width=60% height=60%}

</center>

## QC for BEAGLE sample albino coat color  
...
albino_outliers <- read.table(paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_QC_coatcolor_albino_snp_1_151097606.csv"), stringsAsFactors=FALSE,
                        header=TRUE,  strip.white=TRUE,sep=",")
```
...
<center>
![Figure 17: BEAGLE genotypes PC1 vs. PC2 colored by library prep](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_sample_PC2_vs_PC1_Library_ID.png")`){#id .class width=55% height=55%} 

![Figure 18: BEAGLE genotypes PC1 vs. PC2 colored by project name](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_sample_PC2_vs_PC1_Sample_Project.png")`){#id .class width=60% height=60%} 

![Figure 19: BEAGLE genotypes PC1 vs. PC2 colored by family](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_sample_PC2_vs_PC1_Family.png")`){#id .class width=55% height=55%} 

![Figure 20: BEAGLE genotypes PC1 vs. PC3 colored by family](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_sample_PC3_vs_PC1_Family.png")`){#id .class width=55% height=55%} 

![Figure 21: BEAGLE genotypes PC1 vs. PC4 colored by family](`r paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_sample_PC4_vs_PC1_Family.png")`){#id .class width=55% height=55%} 
...
QC_coatcolor_albino <- read.table(paste0(dir_path, "/", ref,"/results/genotype_result/beagle_result/after_beagle_QC_coatcolor_albino_snp_1_151097606.csv"), stringsAsFactors=FALSE,
                        header=TRUE,  strip.white=TRUE,sep=",")
...
```
8. BEAGLE Imputation  
Software used: [BEAGLE `r unique(software[software$V1 == "Beagle", ]$V2)`](https://faculty.washington.edu/browning/beagle/b4_1.html)  
Command used:  
```
java -XX:+AggressiveOpts -XX:+AggressiveHeap \
    -jar beagle.27Jan18.7e1.jar \
    gt=chr${ch}_stitch.vcf.gz \
    gprobs=true \
    ne=150 \
    nthreads=24 \
...
## Preparation for BEAGLE imputation genetic map
HS rat genetic map:```/projects/ps-palmer/reference_genomes/rat/genetic_map_by_Mike/HS_Rat_Genetic_Map/predicted_cM_SNParray/map_files```  

Citation: Littrell, J., S.-W. Tsaih, A. Baud, P. Rastas, L. Solberg-Woods et al., 2018 A High-Resolution Genetic Map for the Laboratory Rat. G3 (Bethesda) 8: 2241–2248. doi:10.1534/g3.118.200187   

Use the following R code to reorganize the genetic map for BEAGLE.  
```
for(c in c(1:20, "X")){
    fp<- paste(fp, "map_files/chr", sep="")
    mp<- cbind(chr=paste("chr",c,sep=""), snp=paste("chr",c,"_",mp[,1],sep=""), 
        mp[,c(3,1)])
    write.table(mp, file=paste("/beagle_map/chr",c,"_beagle.mp",sep=""),
        row.names=FALSE, col.names=FALSE, quote=FALSE)
}
```
Example:  
Before: ```40990 0.490095 4.58974178576291```  
After: ```chr1 chr1_40990 4.58974178576291 40990```
